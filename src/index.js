  ...
  return async function request(method, path, bodyObj) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new W3ActionError('TIMEOUT', `${method} ${path} timed out after 30s`)
      }
      throw new W3ActionError('REQUEST_FAILED', `${method} ${path}: ${err.message}`)
    }
    clearTimeout(timer)
    if (res.status === 204) {
      return null
    ...
    if (!res.ok) {
      ...
      throw new W3ActionError('HTTP_ERROR', `${method} ${path} returned ${res.status}: ${msg}`, {
...
function parseBody() {
  ...
  if (!body) throw new W3ActionError('MISSING_INPUT','body input is required for this command')
...
const router = createCommandRouter({
  ...
  'get-autoramp': handler('get-autoramp', async (request) => {
    ...
    if (!autorampId) throw new W3ActionError('MISSING_INPUT','autoramp-id is required')
  ...
  'get-autoramp-by-external-id': handler('get-autoramp-by-external-id', async (request) => {
    ...
    if (!externalId) throw new W3ActionError('MISSING_INPUT','external-id is required')
  ...
  'get-open-banking-payment': handler('get-open-banking-payment', async (request) => {
    ...
    if (!paymentId) throw new W3ActionError('MISSING_INPUT','payment-id is required')
  ...
  'get-transactions-by-ids': handler('get-transactions-by-ids', async (request) => {
    ...
    if (!transactionIds) throw new W3ActionError('MISSING_INPUT','transaction-ids is required')
  ...
  'get-customer': handler('get-customer', async (request) => {
    ...
    if (!customerId) throw new W3ActionError('MISSING_INPUT','customer-id is required')
  ...
  'get-identification': handler('get-identification', async (request) => {
    ...
    if (!addressId) throw new W3ActionError('MISSING_INPUT','address-id is required (identification ID)')
  ...
  'disable-crypto-address': handler('disable-crypto-address', async (request) => {
    const addressId = core.getInput('address-id') || ''
    if (!addressId) throw new W3ActionError('MISSING_INPUT','address-id is required')
  ...
  'get-auth-code': handler('get-auth-code', async (request) => {
    const addressId = core.getInput('address-id') || ''
    if (!addressId) throw new W3ActionError('MISSING_INPUT','address-id is required (entity ID)')
    return request('GET', `/authentication-codes/entity/${addressId}`)
  }),

  'submit-auth-code': handler('submit-auth-code', async (request) => {
    const addressId = core.getInput('address-id') || ''
    if (!addressId) throw new W3ActionError('MISSING_INPUT','address-id is required (auth code ID)')
  ...
  'get-country-subdivisions': handler('get-country-subdivisions', async (request) => {
    ...
    if (!countryCode) throw new W3ActionError('MISSING_INPUT','country-code is required')
  ...
  'update-webhook': handler('update-webhook', async (request) => {
    ...
    if (!webhookId) throw new W3ActionError('MISSING_INPUT','webhook-id is required')
  ...
  'sandbox-update-transaction': handler('sandbox-update-transaction', async (request) => {
    const addressId = core.getInput('address-id') || ''
    const sandboxStatus = core.getInput('sandbox-status') || ''
    if (!addressId) throw new W3ActionError('MISSING_INPUT','address-id is required (transaction ID)')
    return request(
      'PUT',
